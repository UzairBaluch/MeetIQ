...
Horizontal 4-layer layout (Express + PostgreSQL/Prisma):

```
backend/prisma/
  schema.prisma      # Prisma data model for PostgreSQL
  migrations/        # SQL migrations generated by Prisma
  seed.ts            # seed entrypoint

backend/src/
  app.ts             # Express app factory
  server.ts          # HTTP listen, DB connect, Socket.IO bootstrap
  ...
  services/          # business rules — no DB client access here
  repositories/      # persistence layer using Prisma client
  middleware/        # auth, errors, logging, limits
  utils/             # shared helpers & infra glue (e.g. socket registry)
```

Name files consistently by resource (`user.routes.ts`, `user.controller.ts`, `user.service.ts`, `user.repository.ts`).

**Validation:** HTTP-only Zod (etc.) stays co-located as `controllers/<resource>.schema.ts` or under `utils/` when shared — not a separate top-level `schemas/` folder. **Database model** lives in `prisma/schema.prisma`.
